...
type ReportRow = {
  ...
  weeklyPlan: { id: number; week_number: number; status: string } | null;
};
...
export default function SupervisorAttendancePage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get<ReportRow[]>("/supervisor/weekly-reports");
      setRows(res.data);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Attendance & execution</h1>
        <p className="mt-1 text-sm text-slate-500">
          Per-week attendance and execution notes for students placed at your company (FR-6.5).
        </p>
      </div>
            ...
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className="border-b border-slate-100 bg-slate-50/80">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold text-slate-700 text-right">Save</th>
                </tr>
              </thead>
