...
export default function HodPlacementsPage() {
  ...
  const sendProposal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProposalWeeks("");
      setProposalOutcomes("");
      setToast({ show: true, message: ids.length > 1 ? `${ids.length} proposals sent` : "Proposal sent successfully" });
      await load();
  ...
  return (
    <div className="space-y-6 pb-8">
      <HodPageHero
        badge="Placements"
        title="Placements"
        description="Send university-initiated proposals to verified companies and track responses."
        action={
        ...
        <>
          {activeTab === "send" && (
            <div className="space-y-6">
              {studentsNeedingReassignment.length > 0 && (
                <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/50 dark:bg-amber-900/20">
                  <h2 className="text-base font-bold text-amber-900 mb-3 dark:text-amber-200">Students needing reassignment</h2>
                  <div className="space-y-2">
                    {studentsNeedingReassignment.map((s) => (
                      <div key={s.id} className="flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3 dark:border-amber-900/50 dark:bg-slate-900">
                        ...
                        <button type="button" onClick={() => { setReassignStudent(s); setReassignCompanyId(""); setReassignWeeks(""); }}
          ...
          {activeTab === "placed" && (
            placedStudents.length === 0 ? (
              ...
              <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900/50 dark:bg-emerald-900/10">
                <div className="flex items-center gap-2 mb-4">
                  <CheckCircle2 className="h-5 w-5 text-emerald-600" />
                  <h2 className="text-base font-bold text-emerald-900 dark:text-emerald-200">Placed students ({placedStudents.length})</h2>
                </div>
                <div className="space-y-2">
                  {placedStudents.map((s) => (
                    <div key={s.id} className="flex items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-white px-4 py-3 dark:border-emerald-900/50 dark:bg-slate-900">
                      ...
                      <div className="text-right shrink-0">
                        {s.companyName && <p className="text-sm font-medium text-emerald-700">{s.companyName}</p>}
                        <span className="inline-block rounded-full bg-emerald-100 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700">Placed</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {reassignStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl mx-4 space-y-4 dark:bg-slate-900 dark:border dark:border-slate-700">
            ...
            <p className="text-sm text-slate-500">Assign <strong>{reassignStudent.user.full_name}</strong> to a new company.</p>
            <form onSubmit={(e) => void sendReassignment(e)} className="space-y-3">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setReassignStudent(null)}
                  className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200">
                  Cancel
                </button>
