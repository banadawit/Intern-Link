    ...
    <div className="space-y-8 pb-8 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <div
            key={s.label}
            className="group flex items-center gap-4 rounded-2xl border border-border-default bg-white p-5 shadow-sm transition-all duration-200 hover:border-primary-200 hover:shadow-md dark:bg-slate-900"
          >
            <div
              className={cn(
      ...
      <section className="card overflow-hidden p-0 transition-shadow duration-200 hover:shadow-md">
            ...
            <tbody className="divide-y divide-border-default">
              {latestPlans.length === 0 ? (
                ...
                latestPlans.map((plan) => (
                  <tr key={plan.id} className="bg-white transition-colors hover:bg-slate-50/80 dark:bg-slate-900 dark:hover:bg-slate-800/80">
        ...
        <div className="space-y-3 p-4 md:hidden">
          {latestPlans.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-500 dark:text-slate-400">No weekly plans yet.</p>
          ) : (
            latestPlans.map((plan) => (
              <div
                key={plan.id}
                className="flex items-center justify-between gap-3 rounded-xl border border-border-default bg-white p-4 shadow-sm dark:bg-slate-900"
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">{new Date(plan.submittedAt).toLocaleDateString()}</p>
                  </div>
                </div>
                <Link
                  href="/student/plans"
