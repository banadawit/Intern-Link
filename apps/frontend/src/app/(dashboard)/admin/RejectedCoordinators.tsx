        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-200">
            {!loading && coordinators.length === 0 && (
              <tr><td colSpan={4} className="px-6 py-12 text-center text-sm text-slate-500">No rejected coordinators.</td></tr>
            )}
            {coordinators.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-lg bg-red-50 text-red-500"><User className="w-4 h-4" /></div>
                    <div>
