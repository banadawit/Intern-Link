...
interface ConfirmState {
  type: ActionType;
  coordinator: ApprovedCoordinator | null;
  reason: string;
}
...
const ApprovedCoordinators = () => {
  ...
  const [confirm, setConfirm] = useState<ConfirmState>({ type: null, coordinator: null, reason: "" });
  ...
  const openConfirm = (type: ActionType, coordinator: ApprovedCoordinator) =>
    setConfirm({ type, coordinator, reason: "" });

  const closeConfirm = () => setConfirm({ type: null, coordinator: null, reason: "" });
  ...
  const handleAction = async () => {
    ...
    try {
      if (confirm.type === "suspend") {
        await api.post(`/admin/coordinators/${uid}/suspend`, { reason: confirm.reason });
        showToast("Coordinator suspended.", true);
      } else if (confirm.type === "deactivate") {
        await api.post(`/admin/coordinators/${uid}/reject`, { reason: confirm.reason });
        showToast("Coordinator deactivated.", true);
      }
      closeConfirm();
      load();
    } catch (e: any) {
  ...
  const actionMeta: Record<NonNullable<ActionType>, { label: string; color: string; icon: React.ReactNode; desc: string }> = {
    suspend: {
      ...
      desc: "The coordinator will lose platform access temporarily and can be reactivated later.",
    },
    deactivate: {
      label: "Deactivate",
      color: "bg-orange-600 hover:bg-orange-700",
      icon: <XCircle className="w-4 h-4" />,
      desc: "The coordinator's access will be revoked. They will need to re-apply.",
    },
  };
  ...
  return (
    <div className="relative">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {actionMeta[confirm.type].label} Coordinator
              </h3>
            </div>
      ...
      <div className="card overflow-hidden">
        {loading && (
          <p className="p-6 text-sm text-slate-500 dark:text-slate-400" role="status">Loading approved coordinators…</p>
        )}
