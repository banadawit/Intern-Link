...
const CoordinatorApprovals = ({ onActionComplete, hideHero = false }: Props) => {
  const t = useTranslations("AdminPortal.coordinatorApprovals");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get<PendingCoordinator[]>("/admin/pending-coordinators");
  ...
  const handleApprove = async (userId: number) => {
    ...
    try {
      ...
      setToast({ show: true, message: ta("toastApprovedCoord") });
      await load();
      onActionComplete?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {!hideHero && (
        <AdminPageHero
          badge={ta("tabs.coordinators.label")}
          title={ta("tabs.coordinators.title")}
          description={ta("tabs.coordinators.description")}
        />
      )}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-100 text-slate-500 text-xs uppercase tracking-wider dark:bg-slate-800 dark:text-slate-400">
                <th className="px-6 py-4 font-semibold">{t("tableCoordinator")}</th>
                <th className="px-6 py-4 font-semibold">{t("tableUniversity")}</th>
                ...
                <tr key={c.userId} className="hover:bg-slate-50 transition-colors dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                      ...
                      {c.pending_university_name || <span className="text-slate-400 italic">{ta("notProvided")}</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4">
