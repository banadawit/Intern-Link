...
const Sidebar = ({ activeView, onNavigate, pendingCount = 0, pendingCoordinatorCount = 0, pendingSupervisorCount = 0 }: Props) => {
  ...
  return (
    <aside className="sticky top-0 z-40 flex w-full shrink-0 flex-col border-b border-slate-200 bg-white shadow-sm lg:h-screen lg:w-64 lg:border-b-0 lg:border-r lg:shadow-none dark:border-slate-800 dark:bg-slate-950">
      {showLogout && (
        <LogoutModal onConfirm={handleLogout} onCancel={() => setShowLogout(false)} />
      )}
      <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-4 lg:px-6 dark:border-slate-800">
          ...
          <button
            ...
            className={cn(
              ...
              activeView === item.view
                ? "bg-teal-50 text-teal-700 shadow-sm ring-1 ring-teal-100 dark:bg-teal-900/30 dark:text-teal-200 dark:ring-teal-800"
                : "text-slate-500 hover:bg-slate-100 hover:text-slate-700 active:scale-[0.98] dark:text-slate-400 dark:hover:bg-slate-900 dark:hover:text-slate-200"
            )}
          >
      ...
      <div className="flex items-center justify-between gap-2 border-t border-slate-200 px-3 py-3 lg:hidden dark:border-slate-800">
