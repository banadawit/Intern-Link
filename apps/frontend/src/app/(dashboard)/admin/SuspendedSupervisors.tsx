...
const SuspendedSupervisors = () => {
  ...
  const [confirm, setConfirm] = useState<{ type: ActionType; supervisor: SuspendedSupervisor | null }>({ type: null, supervisor: null });
  ...
  const handleAction = async () => {
    ...
    try {
      if (confirm.type === "reactivate") {
        await api.post(`/admin/supervisors/${uid}/reactivate`);
        showToast("Supervisor reactivated.", true);
      }
      setConfirm({ type: null, supervisor: null });
      load();
    } catch (e: any) {
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {confirm.type === "reactivate" ? "Reactivate" : "Delete"} Supervisor
              </h3>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
              ...
              <br />
              The supervisor will regain full platform access.
            </p>
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setConfirm({ type: null, supervisor: null })}
                disabled={acting}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {!loading && supervisors.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-slate-500 dark:text-slate-400">No suspended supervisors.</td></tr>
              )}
              {supervisors.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{format(new Date(s.user.created_at), "MMM d, yyyy")}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setConfirm({ type: "reactivate", supervisor: s })}
