...
export default function HodProposalTrackerTable({ proposals }: Props) {
  const t = useTranslations("HodPortal.proposalTracker");
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-slate-100 dark:border-slate-700">
        <table className="w-full min-w-[640px] text-sm">
          <thead className="border-b border-slate-100 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800/70">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colStudent")}</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colCompany")}</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colStatus")}</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colWeeks")}</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colSubmitted")}</th>
            </tr>
          </thead>
        ...
        {proposals.length === 0 && (
          <p className="py-8 text-center text-sm text-slate-500 dark:text-slate-400">{t("noProposals")}</p>
        )}
      </div>
    </section>
  );
