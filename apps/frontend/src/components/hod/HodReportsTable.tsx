...
export default function HodReportsTable({ reports }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 dark:border-slate-700 dark:bg-slate-900">
      <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">{t("tableTitle")}</h2>
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t("tableSubtitle")}</p>
      <div className="mt-4 overflow-x-auto rounded-xl border border-slate-100 dark:border-slate-700">
        <table className="w-full text-sm">
          <thead className="border-b border-slate-100 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800/70">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colStamped")}</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{t("colDownload")}</th>
            </tr>
          </thead>
        ...
        {reports.length === 0 && (
          <p className="py-8 text-center text-sm text-slate-500 dark:text-slate-400">{t("noReports")}</p>
        )}
      </div>
    </section>
  );
